...
* @Last Modified time: 2020-08-08 23:03:00
*/
...
SC-3:
"fkjjlbhkbbefinemajmoebhjbkojmcaehiibankkomghncojbjgedebjfdocjhclmbalebladkcaidacaiiokemdmaabjalmbgggjjfjfedegmnkidceogjdgncmlhodkcmjkfolgfnaklkjbocjhhakgmigkcmilbikbhjcgz"
...
### Still working on this problem, maybe should use DFS instead of DP. DP seems not a good solution to this question. ###
        	...
        	for(int j = 0; j <= i; j++) {
        		if((!dp.get(j).isEmpty()) && wordDict.contains(s.substring(j, i+1))) { 
        			for(int k = 0; k < dp.get(j).size(); k++) {
        				String prevWordBreak = dp.get(j).get(k);
...
// Solution 2 - HashMap instead of arraylist
